...
namespace Cassandra
{
    ...
    internal class Connection : IDisposable
    {
        ...
        public virtual int TimedOutOperations
        {
            ...
            get { return _timedOutOperations; }
        }
        ...
        internal void CancelPending(Exception ex, SocketError? socketError = null)
        {
            ...
            if (!wasClosed)
            {
                Logger.Info("Canceling in Connection {0}, {1} pending operations and write queue {2}", Address, /*Thread.VolatileRead(ref _inFlight)*/ _inFlight, _writeQueue.Count);
                if (socketError != null)
                {
