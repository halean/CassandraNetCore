...
namespace Cassandra.Data.Linq
{
    ...
    internal class LinqAttributeBasedTypeDefinition : ITypeDefinition
    {
        ...
        public LinqAttributeBasedTypeDefinition(Type type, string tableName, string keyspaceName)
        {
            ...
            {
                TableName = type.Name;
                var tableAttribute = (TableAttribute)type.GetTypeInfo().GetCustomAttributes(typeof(TableAttribute), true).FirstOrDefault();
            ...
            if (type.GetTypeInfo().GetCustomAttributes(typeof(CompactStorageAttribute), true).FirstOrDefault() != null)
            {
                CompactStorage = true;
            }
            if (type.GetTypeInfo().GetCustomAttributes(typeof(AllowFilteringAttribute), true).FirstOrDefault() != null)
            {
                AllowFiltering = true;
            }
        }
        ...
        {
            if (type.GetTypeInfo().GetCustomAttributes(typeof(Cassandra.Data.Linq.TableAttribute), true).Count() > 0)
            {
